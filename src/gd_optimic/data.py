...
class ObservationOperator:
    ...
    def align_grid(
        ...
        This implements nearest-neighbor interpolation for SSH along-track data
        and bilinear interpolation for gridded SST data.
        ...
        lat_match = np.allclose(source_lat, target_lat, rtol=1e-5)
        lon_match = np.allclose(source_lon, target_lon, rtol=1e-5)
        
        if lat_match and lon_match:
            # Grids match - no regridding needed
            return source_ds
        
        # Grids differ - perform regridding
        # Use bilinear for smooth fields (SST), nearest-neighbor preserves sparse obs (SSH)
