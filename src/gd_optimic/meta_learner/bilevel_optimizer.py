...
class BiLevelICOptimizer:
    ...
    def step(
        ...
        **PERF Phase (m >= grad_align_steps + grad_trans_steps):**
        ...
        gradient_prev = gradient_prev.to(self.device) # Ensure gradient_prev is detached to avoid OOM
        ...
        for m in range(self.num_meta_steps):
            ...
            # ============================================================================
            # STEP 3: Compute meta-loss components
            # ============================================================================
            # Regularization: L2 penalty on network_s parameters
            # This prevents parameter explosion and encourages smaller, smoother updates
            l_reg = self.lambda_reg * sum((p ** 2).sum() for p in self.network_s.parameters())

            # Alignment loss: How well does the predicted update align with observed gradient?
            # L_align = ||S(θ, x) - ∇_x J||_2
            # This measures gradient prediction accuracy
            # ic_update already has requires_grad=True from predict_update()
            l_align = (gradient_prev - ic_update).pow(2).sum()  # Mean squared error between predicted update and observed gradient
            ...
            # ============================================================================
            # STEP 4: Combine losses by phase and compute gradients
            # ============================================================================
            ...
            l_perf_weighted = self.w_perf * loss_current
            l_combined = l_align_weighted + l_perf_weighted + l_reg
            ...
            # Memory cleanup
            del loss_current, l_reg, improvement, y_hat_steps_new, ic_update
    ...
    def predict_update(self, x: torch.Tensor) -> torch.Tensor:
        """
